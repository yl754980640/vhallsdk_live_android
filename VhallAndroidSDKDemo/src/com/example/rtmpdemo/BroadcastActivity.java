...
//		}
	...
	private void startBrocast() {
		...
		if (Constants.sdk_type == Constants.TYPE_SELF) {//自助式，获取直播地址
			mProcessDialog.show();
			ZReqEngine.start2(roomid, Constants.APP_KEY, Constants.APP_SECRET_KEY, access_token, new ReqCallback() {
		...
		} else if (Constants.sdk_type == Constants.TYPE_STREAM) {//流式，地址固定，token区分
	...
	private void stopBroadcast() {
		if (Constants.sdk_type == Constants.TYPE_SELF) {//自助式请求服务器，结束直播，生成回放
			mProcessDialog.show();
			ZReqEngine.stop(roomid, Constants.APP_KEY, Constants.APP_SECRET_KEY, access_token, new ReqCallback() {
		...
		} else if (Constants.sdk_type == Constants.TYPE_STREAM) {//流式，直接结束
			stopPublish();
		}
	}
